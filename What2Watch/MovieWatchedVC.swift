    ...
    */
    ...
    func tableView(tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        if self.movieWatched.count == 0 {
            ...
            emptyLabel.text = "Currently you not have any watched movie!"
    ...
    func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let IsLike = self.movieWatched[indexPath.row]["status"] as? String where IsLike == "like" {
            cell.lblValue.text = "Liked"
            cell.lblValue.textColor = AppState.sharedInstance.clrYellow
        } else {
            cell.lblValue.text = "Disliked"
            cell.lblValue.textColor = UIColor.lightGrayColor()
        }
